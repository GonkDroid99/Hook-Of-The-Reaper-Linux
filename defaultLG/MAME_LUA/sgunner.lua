...
-- Memory Values								 	--
------------------------------------------------------
_Credits_MemoryAddress = 0x100121
_StartLamp = {0, 0}
_PlayerStatus = {0, 0}
_PlayerLife = {0, 0}
_PlayerAmmo = {0, 0}
...
function Compute_Outputs() 
	...
	for i=0, 1 do	
		...
		-- Genuine Outputs
--		manager.machine.output:set_value("P" .. (i + 1) .. "_RealRecoil", mem:read_u16(0x1002F2  + (i * 0x2)))
--		manager.machine.output:set_value("P" .. (i + 1) .. "_LmpStart", _StartLamp[i])
	
		GameStatus = mem:read_u8(0x1002E7) 
...
--		_PlayerStatus[i] = mem:read_u8(0x00502088 + (i * 0x04))
		_PlayerLife[i] = mem:read_u8(0x100125 + (i * 0x2))
		_PlayerAmmo[i] = mem:read_u16(0x100324 + (i * 0x2))	
