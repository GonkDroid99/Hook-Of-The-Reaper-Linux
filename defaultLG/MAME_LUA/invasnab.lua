...
--		manager.machine.output:set_value("P" .. (i + 1) .. "_LmpStart", _StartLamp[i])
		manager.machine.output:set_value("P" .. (i + 1) .. "_RocketRecoil", _PlayerRockets[i]) -- Invasion is weird, rockets are infinite, but seem to trigger this output
-- 00A032
-- A035 rockets, maybe?	
		GameStatus = mem:read_u16(0xA032) -- not needed for Invasion
...
--		end
		...
		_LastRocket[i] = _PlayerRockets[i]
...
--	bOutputs = mem:read_u8(0x005D0540)
	...
	for i=0, 1 do
		_PlayerRockets[i] = mem:read_u16(0xA032 + (i * 0x36))
		_PlayerLife[i] = mem:read_u32(0xA015 + (i * 0x36))
		_PlayerAmmo[i] = mem:read_u8(0xA02B + (i * 0x36))	
...
--		s:draw_text(0, 15 + (i * 60), string.format("P" .. (i + 1) .. "_StartLamp : %d", _StartLamp[i]), 0xffff0000, 0xff000000)
		s:draw_text(0, 30 + (i * 60), string.format("P" .. (i + 1) .. "_Rockets : %d", _PlayerRockets[i]), 0xffff0000, 0xff000000)
...
manager.machine.output:set_value("P1_RocketRecoil", 0)
manager.machine.output:set_value("P2_RocketRecoil", 0)
