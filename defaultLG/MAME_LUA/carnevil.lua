...
-- Also fixed the Life memory location, so now Life and Damaged outputs works
...
-- Memory Values								 	--
------------------------------------------------------
_Credits_MemoryAddress = 0x001DE8D8
_StartLamp = {0, 0}
_PlayerStatus = {0, 0}
_PlayerLife = {0, 0}
_PLifeInt = {0, 0}
...
--		GameStatus = mem:read_u8(0x0050003D) -- not needed for CarnEvil
...
--	bOutputs = mem:read_u8(0x005D0540) -- not known
--	0x001E0F86
	
	for i=0, 1 do
--		_PlayerStatus[i] = mem:read_u8(0x00502088 + (i * 0x04)) -- not used
--		_PlayerLife[i] = mem:read_u8(0x001E1034 + (i * 0x118)) -- no working need to figure out ...
		_PLifeInt[i] = mem:read_u32(0x001E0F84 + (i * 0x120))
		_PlayerAmmo[i] = mem:read_u8(0x001E0FFC + (i * 0x120))	-- correct
	--	maincpu.pb@9000876F
--		_StartLamp[i] = bOutputs >> (2 + i) & 0x01
	
		--Conversion from Unsigned Integer to 32bit float
		--Health is 900.0, and then goes down to 0
		local hex_padded = string.format("%08x", _PLifeInt[i])
		local int_val = tonumber(hex_padded, 16)
		local packed_bytes = string.pack("I4", int_val)
		local float_val = string.unpack("f", packed_bytes)
	
		_PlayerLife[i] = float_val
...
--		s:draw_text(0, 30 + (i * 60), string.format("P" .. (i + 1) .. "_STATUS : %d", _PlayerStatus[i]), 0xffff0000, 0xff000000)
		s:draw_text(0, 45 + (i * 60), string.format("P" .. (i + 1) .. "_Life : %f", _PlayerLife[i]), 0xffff0000, 0xff000000)
