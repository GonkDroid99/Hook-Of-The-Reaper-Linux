...
-- Memory Values								 	--
------------------------------------------------------
_Credits_MemoryAddress = 0x17D66
_StartLamp = {0, 0}
_PlayerStatus = {0, 0}
_PlayerLife = {0, 0}
_PlayerAmmo = {0, 0}
...
--		manager.machine.output:set_value("P" .. (i + 1) .. "_LmpStart", _StartLamp[i])
		...
		GameStatus = mem:read_u8(0x17D1B) -- seems to go to 1 during mini-game (including attract, but there ammo stays at 0)
...
--		_PlayerStatus[i] = mem:read_u8(0x00502088 + (i * 0x04))
		_PlayerLife[i] = mem:read_u8(0x17D4B + (i * 0xC))
		_PlayerAmmo[i] = mem:read_u16(0x18D96 + (i * 0x10))	
...
--		s:draw_text(0, 15 + (i * 60), string.format("P" .. (i + 1) .. "_StartLamp : %d", _StartLamp[i]), 0xffff0000, 0xff000000)
		s:draw_text(0, 30 + (i * 60), string.format("P" .. (i + 1) .. "_STATUS : %d", GameStatus), 0xffff0000, 0xff000000)
